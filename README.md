<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <meta name="description" content="Masajes eróticos, tántricos y sensitivos para mujeres. Vive una experiencia única de conexión íntima femenina.">
  <title>PLACER PRIVADO – Masajes de Conexión Íntima Femenina</title>
  <script src="https://cdn.tailwindcss.com"></script>
  <link rel="icon" href="/assets/favicon.png" />
  <style>
    body {
      background-color: #FFF8E7; /* Marfil */
    }
    .hero {
      background: linear-gradient(rgba(1, 68, 33, 0.8), rgba(1, 68, 33, 0.8)), url('/assets/fondo.jpg');
      background-size: cover;
      background-position: center;
    }
  </style>
</head>
<body class="text-gray-800 font-sans leading-relaxed">

  <!-- Hero -->
  <section class="hero text-white py-20 px-6 text-center">
    <h1 class="text-4xl md:text-6xl font-bold mb-4 tracking-wide">PLACER PRIVADO</h1>
    <h2 class="text-xl md:text-2xl italic">Masajes de Conexión Íntima Femenina</h2>
    <p class="mt-6 text-lg max-w-2xl mx-auto">Masaje Erótico, Tántrico, Sensitivo para mujeres. Disfruta una experiencia multisensorial a domicilio, única y transformadora.</p>
  </section>

  <!-- Sección: Quién Soy -->
  <section class="px-6 py-12 max-w-4xl mx-auto">
    <h2 class="text-3xl text-green-900 font-bold mb-4">Quién Soy</h2>
    <p>Soy un masoterapeuta profesional, hombre de 47 años, con experiencia desde 2023 ofreciendo este tipo de masaje exclusivo para mujeres...</p>
    <!-- Puedes seguir con el mismo texto completo que me diste, está abajo -->
  </section>

  <!-- Sección: Experiencia Multisensorial -->
  <section class="bg-green-50 px-6 py-12">
    <div class="max-w-4xl mx-auto">
      <h2 class="text-3xl text-green-900 font-bold mb-4">Una experiencia multisensorial</h2>
      <p>Cada sesión se ambienta con delicadeza: velas, música suave, calefactor para templar el ambiente, aromaterapia embriagadora, geles estimulantes y juguetes opcionales...</p>
    </div>
  </section>

  <!-- Sección: Cobertura -->
  <section class="px-6 py-12 max-w-4xl mx-auto">
    <h2 class="text-3xl text-green-900 font-bold mb-4">Cobertura de Atención</h2>
    <p>Atiendo a domicilio en toda la Región Metropolitana... y también en la Quinta Región.</p>
  </section>

  <!-- Sección: Beneficios -->
  <section class="bg-yellow-50 px-6 py-12">
    <div class="max-w-4xl mx-auto">
      <h2 class="text-3xl text-yellow-700 font-bold mb-4">Beneficios que trascienden el cuerpo</h2>
      <ul class="list-disc pl-6 space-y-2">
        <li>Desbloqueo de emociones profundas y tensiones sexuales antiguas</li>
        <li>Aumento de la autoestima, el deseo y la sensualidad natural</li>
        <li>Profunda reconexión emocional, sexual y espiritual</li>
        <!-- Agrega los demás beneficios -->
      </ul>
    </div>
  </section>

  <!-- Sección: Servicios y Tarifas -->
  <section class="px-6 py-12 max-w-4xl mx-auto">
    <h2 class="text-3xl text-red-800 font-bold mb-4">Servicios y Tarifas</h2>
    <p class="mb-4">✔ Experiencia sensitiva breve: desde $35.000 (30 min)<br/>
    ✔ Masaje tántrico o erótico completo: $50.000 (final feliz incluido)<br/>
    ✔ Masaje con extensión sensorial profunda: a convenir</p>
  </section>

  <!-- Sección: Testimonios -->
  <section class="bg-pink-50 px-6 py-12">
    <div class="max-w-4xl mx-auto">
      <h2 class="text-3xl text-pink-800 font-bold mb-4">Testimonios</h2>
      <blockquote class="italic border-l-4 border-pink-400 pl-4 mb-6">“Estuve con miedo, tímida... Me sentí con fuego, con libertad...” — Mujer, 50 años</blockquote>
      <blockquote class="italic border-l-4 border-pink-400 pl-4 mb-6">“Nunca pensé que un masaje a domicilio pudiera ser tan excitante...” — Mujer, 43 años</blockquote>
      <!-- Agrega más testimonios aquí -->
    </div>
  </section>

  <!-- Sección: Preguntas Frecuentes -->
  <section class="px-6 py-12 max-w-4xl mx-auto">
    <h2 class="text-3xl text-green-900 font-bold mb-4">Preguntas Frecuentes</h2>
    <p class="mb-4"><strong>¿Hay penetración digital o con pene?</strong> Solo con consentimiento mutuo. La penetración con pene no es habitual.</p>
    <p class="mb-4"><strong>¿Dónde se realiza el servicio?</strong> A domicilio, hoteles, oficinas, Airbnb, o en mi depto en casos excepcionales.</p>
    <!-- Agrega más preguntas -->
  </section>

  <!-- Sección: Recomendaciones -->
  <section class="bg-gray-50 px-6 py-12">
    <div class="max-w-4xl mx-auto">
      <h2 class="text-3xl text-gray-800 font-bold mb-4">Recomendaciones para tu sesión</h2>
      <ul class="list-disc pl-6 space-y-2">
        <li>Hidratación previa</li>
        <li>No comer justo antes del masaje</li>
        <li>Informar condiciones de salud importantes</li>
        <!-- Continúa con todas tus recomendaciones -->
      </ul>
    </div>
  </section>

  <!-- Sección: Contacto / Cierre -->
  <section class="bg-green-900 text-white py-12 px-6 text-center">
    <h2 class="text-3xl font-bold mb-4">Confía en mis manos</h2>
    <p class="mb-6">Donde estés, ahí estaré contigo. Un encuentro sagrado contigo misma.</p>
    <a href="https://wa.me/569XXXXXXX" class="bg-yellow-500 text-green-900 font-bold py-3 px-6 rounded-full hover:bg-yellow-400">Agenda por WhatsApp</a>
  </section>

</body>
</html>
